...
def test_accept_shared_tasks_all_deleted_returns_404(mock_redis, mock_db, mock_wake, sample_payload):
...
@patch("routers.action_items.action_items_db")
@patch("routers.action_items.redis_db")
def test_accept_shared_tasks_all_locked_returns_402(mock_redis, mock_db, mock_wake, sample_payload):
    mock_redis.verify_task_share_token.return_value = True
    mock_db.get_action_items_by_ids.return_value = [
...
@patch("routers.action_items.action_items_db")
@patch("routers.action_items.redis_db")
def test_accept_shared_tasks_self_accept_forbidden(mock_redis, mock_db, sample_payload):
    with pytest.raises(HTTPException) as exc_info:
...
@patch("routers.action_items.action_items_db")
@patch("routers.action_items.redis_db")
def test_accept_shared_tasks_invalid_or_used_token_conflict(mock_redis, mock_db, sample_payload):
...
@patch("routers.action_items.action_items_db")
@patch("routers.action_items.redis_db")
def test_accept_shared_tasks_success_creates_and_wakes(mock_redis, mock_db, mock_wake, sample_payload):
    mock_redis.verify_task_share_token.return_value = True
    mock_db.get_action_items_by_ids.return_value = [
...
@patch("routers.action_items.action_items_db")
@patch("routers.action_items.redis_db")
def test_accept_shared_tasks_creation_failure_rolls_back_token(mock_redis, mock_db, mock_wake, sample_payload):
    mock_redis.verify_task_share_token.return_value = True
    mock_db.get_action_items_by_ids.return_value = [{"id": "task-1", "description": "Do homework", "is_locked": False}]
    mock_db.create_action_item.side_effect = RuntimeError("Database down")

    with pytest.raises(RuntimeError):
        accept_shared_action_items(payload=sample_payload, uid="recipient-user-456")

    mock_redis.undo_accept_task_share.assert_called_once_with(sample_payload.token, sample_payload.task_ids)
    mock_wake.assert_not_called()
